...
//正则表达式插入连接符“.”
string nfadfa::insert_concat(string regexp){
    string ret="";
    char c,c2;
...
string nfadfa::regexp_to_postfix(string regexp)
{
    string postfix="";
    stack<char> op;
    char c;
...
void nfadfa::concatenation()//处理连接
{
    int d = st.top(); st.pop();
    int c = st.top(); st.pop();
    int b = st.top(); st.pop();
...
void nfadfa::postfix_to_nfa(string postfix)
{
...
void nfadfa::show_nfa(string zzz)
{
    string s = zzz;
    s =s+ "\\graph\\nfa.dot";
    cout<<s;
...
void nfadfa::getNeighbor(int v, char c,set<int>&di)
{
...
void nfadfa::epsilon_closure(int v,set<int>&si)
{
    Edge *p=nfa.NodeTable[v].adj;
    while(p!=NULL)
    {
...
void nfadfa::nfa_to_dfa(set<int>&si)
{
    map<set<int>, int> mp;
    mp[si]=-1;
    queue<set<int> > que;
    ...
    epsilon_closure(start_state,si);//起始点集合
    if(mp.count(si)==0){
        mp[si]=ct++;
        que.push(si);
        dfa.insertVertex();
...
void nfadfa::show_dfa(string zzz)
{
    string s = zzz;
    s =s+ "\\graph\\dfa.dot";
    ofstream out(s);
...
int nfadfa::dfa_transform(int v, char c, map<int,vector<int>> mp_1)
{
    ...
    int q; //所在集合在临时表中的序号
    for(int i=0;i<int(mp_1.size());i++)
    {
        for(int j=0;j<int(mp_1[i].size());j++)
        {
            if(mp_1[i][j]==node)
                q = i;
        }
...
bool nfadfa::if_equal(int v1, int v2, vector<char> chars, map<int,vector<int>> mp_1)
{
...
void nfadfa::minimize_dfa()
{
    ...
    while(!states.empty())
    {
        ...
        vector<int> s= states.front();//取出第一个终态集合
        states.pop();
        pop_num++;

        if(s.empty()) continue;
        int sz= s.size();//记录集合大小
        if(sz==1)//如果集合大小为1，map[1]=该集合，直接将集合加入mini_dfa的映射表
        {
            mp[ct++]= s;
            continue;
        }
        ...
        for(int i=1;i<sz;i++)
        {
            //比较第一个结点和第i个结点的转换结果，如果两个结点的转换结果不一样，则需要划分
            if(if_equal(s[i],node,chars,mp_1)==false)
            {
                int cur_node =s[i];
                vector<int> cur_states;
                for(vector<int>::iterator it = s.begin()+1; it != s.end();)//遍历整个集合
                {
                ...
                sub_states.push_back(cur_states);//划分出的新集合加入
        ...
        if(ok)
        {
            mp[ct++]=s;//直接将集合加入mini_dfa的映射表
            //输出日志
            cout<<"This is a test:"<<endl;
            cout<<ct-1;
            for(int r=0;r<int(mp[ct-1].size());r++)
            {
                cout<<mp[ct-1][r]<<" ";
            }
            cout<<endl;
        ...
        {
            sub_states.push_back(s);//将划分后s加入
            for(int l=0;l<int(sub_states.size());l++)
            {
                states.push(sub_states[l]);//加入等待划分的队列
                mp_1[ct_1++]=sub_states[l];//更新临时的映射表
            }
            vector<int> n;
            mp_1[pop_num]=n;
        }
...
void nfadfa::show_mini_dfa(string zzz)
{
    string s = zzz;
    s =s+ "\\graph\\mini_dfa.dot";
    ofstream out(s);
    out<<"digraph abc{";
