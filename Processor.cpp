...
void Processor:: showdfa()
{
    if(isleftFA())
    {
        ...
        for(int i=0;i<grammers.size(); i++)
        {
            out<<grammers[i].left<<"[fontcolor=black][shape=circle];"<<endl;
        }
        ...
        for(int i=0;i<grammers.size(); i++)
        {
            ...
            for(; it!= temp.end(); it++)
            {
                for(int i=0;i<it->size();i++)
                {
                    if(it->size()==2)
                    {
                        out<<it->at(0)<<"->"<<grammers[i].left<<"[label="<<it->at(1)<<"];"<<endl;
                    }
                    else
                    {
                        if(flag==0)
                        {
                            out<<"Z"<<"[fontcolor=black][shape=circle];"<<endl;
                            flag=1;
                            out<<"Z"<<"->"<<grammers[i].left<<"[label="<<it->at(0)<<"];"<<endl;
                        }
                    }
        ...
        for(int i=0;i<grammers.size(); i++)
        {
            ...
            for(; it!= temp.end(); it++)
            {
                for(int i=0;i<it->size();i++)
                {
                    if(it->size()==2)
                    {
                        out<<grammers[i].left<<"->"<<it->at(0)<<"[label="<<it->at(1)<<"];"<<endl;
                    }
                    else
                    {
                        if(flag==0)
                        {
                            out<<"Z"<<"[fontcolor=black][shape=circle];"<<endl;
                            flag=1;
                            out<<grammers[i].left<<"->"<<"Z"<<"[label="<<it->at(0)<<"];"<<endl;
                        }
                    }
